...
class File_processing3():
    ...
    def __init__(self):
        ...
        # 保存路径
        self.save_article_path = r'./data/save_path/{}_articles_with_pic_no_multiplex'.format(self.domain_name)
        self.save_img_path = r'./data/save_path/{}_imgs_with_pic_no_multiplex'.format(self.domain_name)
    ...
    def get_article_str(self, paragraph_list, keyword, imgs):
        ...
        for i in range(len(paragraph_list)):
            ...
            if i == 0 or i == len(paragraph_list) - 1:  # 添加首段/尾段
                ...
                article_str += '<p>%s</p>\n' % paragraph_list[i]
                article_str += '<p><img src={imgpath}/%s_imgs_with_pic_no_multiplex/%s></p>\n' % (self.domain_name, imgs[0])  # 注意修改站点名称
            elif i == 3:  # 添加第四段，并插入一张图片
                article_str += '<p>%s</p>\n' % paragraph_list[i]
                article_str += '<p><img src={imgpath}/%s_imgs_with_pic_no_multiplex/%s></p>\n' % (self.domain_name, imgs[1])  # 注意修改站点名称
            else:  # 添加第三段
                article_str += '<p>%s</p>\n' % paragraph_list[i]
        return article_str
    ...
    def splice_article(self, all_articles):
        ...
        for _ in range(len(self.keywords)):
            ...
            try:
                ...
                break
            article_str = self.get_article_str(temp_article, keyword, img)
    ...
    def main(self):
        ...
        self.keywords = self.read_xlsx()[30:80]  # 取关键词
