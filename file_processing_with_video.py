...
class File_processing4():
    ...
    def __init__(self):
        ...
        # 保存路径
        self.save_img_path = r'./data/save_path/{}_imgs_with_videos'.format(self.domain_name)
        self.save_video_path = r'./data/save_path/{}_videos'.format(self.domain_name)
        self.save_article_path = r'./data/save_path/{}_articles_with_videos'.format(self.domain_name)
    ...
    def operate_video(self, filepath):
        """
        处理图片
        :param filepath:
        :return: 所有图片的路径
        ...
        try:
            videos = []
            for file in os.listdir(self.util.to_gbk(filepath)):
                video = file.decode('gbk').encode('utf-8')
                # imgs.append(os.path.join(filepath, img))
                videos.append(video)
            return videos
        except:
            print('operate video error', traceback.print_exc())
    ...
    def get_all_article(self, dir_list):
        '''获取所有的文章列表'''
        all_articles = []
        for floder in dir_list:  # 每个文件夹操作
        ...
        return all_articles, img_list


    def get_article_str(self, paragraph_list, keyword, imgs, video):
        '''对每一段进行拼接，包括插入关键词和插入图片'''
        article_str = '<p><video src={videopath}/%s_videos/%s controls="controls">您的浏览器不支持 video 标签。</video></p>\n' % (self.domain_name, video)
        for i in range(len(paragraph_list)):
            if i == 0 or i == len(paragraph_list) - 1:  # 添加首段/尾段
                paragraph_list[i] = self.util.insert_keyword(keyword, paragraph_list[i])  # 插入关键词
                article_str += '<p>%s</p>\n' % paragraph_list[i]
            elif i == 1:  # 添加第二段，并插入一张图片
                article_str += '<p>%s</p>\n' % paragraph_list[i]
                article_str += '<p><img src={imgpath}/%s_imgs_with_videos/%s></p>\n' % (self.domain_name, imgs[0])
            elif i == 3:  # 添加第四段，并插入一张图片
                article_str += '<p>%s</p>\n' % paragraph_list[i]
                article_str += '<p><img src={imgpath}/%s_imgs_with_videos/%s></p>\n' % (self.domain_name, imgs[1])
            else:  # 添加第三段
                article_str += '<p>%s</p>\n' % paragraph_list[i]
        return article_str


    def splice_article(self, all_articles, img_list):
        # img_list = self.operate_picture(self.read_path + '/image')  # 获取所有图片
        video_list = self.operate_video(self.read_path + '/video')  # 获取所有视频
        # video_list = os.path.pardir(self.read_path + '/video')  # 获取所有视频
        ## 每次循环生成一篇文章
        for _ in range(len(self.keywords)):
            ...
            imgs = random.sample(img_list, 2)  # 随机取两张图
            try:
                video = random.choice(video_list)  # 随机抽取一个视频
            except:
                break
            article_str = self.get_article_str(temp_article, keyword, imgs, video)
    ...
    def main(self):
        ...
        #     shutil.copytree(self.read_path + '/image', self.save_img_path)
        # 拷贝文件夹
        if not os.path.exists(self.save_video_path):
            shutil.copytree(self.read_path + '/video', self.save_video_path)

        self.keywords = self.read_xlsx()[80:90]  # 取关键词
        file_dir_list = self.get_file_dir(self.read_path)  # 获取所有文件夹及文件
        all_articles, img_list = self.get_all_article(file_dir_list)
        self.splice_article(all_articles, img_list)
